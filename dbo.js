 ...
 */
/*This helper is NOT being used by serverJs*/
...
DBO.prototype = {
    ...
    selectDB: function (keyName, callback) {
        ...
        if(!config[keyName]) {
            throw new Error("Error 100: Could not find any database configuration for key '%s'", keyName)
        }
        ...
        if(this.connection) {
            ...
            this.connection.changeDB(config[keyName]).then(function (result) {
    ...
    getInstance: function (config, newInstance) {
        ...
        var selectedDbConfig = config || dbConfigObj[this.selectedConfig || dbConfig.defaultDb];
