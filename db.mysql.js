...
function DB(config) {
  ...
  self.config = config;
  const usePool =
    false && typeof self.usePool === "undefined" ? true : self.usePool;
  const connectNow =
    typeof self.connectNow === "undefined" ? true : self.connectNow;
...
DB.prototype = {
  ...
  connectDB: function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (self.handle) resolve(self.handle);
      else if (self.usePool) self.getPoolHandle().then(resolve, reject);
      else self.connectHandle().then(resolve, reject);
    });
  },
